 ...
 */
export const createMockOrchestratorClient = (): jest.Mocked<OrchestratorClientService> => ({
  // Health
  ...
  startWorkflow: jest.fn().mockResolvedValue({ success: true, mode: 'langgraph' }),
  ...
  // Budget
  getBudget: jest.fn().mockResolvedValue({ totalSpentUsd: 0, enabled: true }),
  getBudgetReport: jest.fn().mockResolvedValue({ status: {}, taskSpending: [] }),

  // Agents & Audit
  getAgents: jest.fn().mockResolvedValue({ agents: [] }),
  getAudit: jest.fn().mockResolvedValue({ entries: [], total: 0 }),
  getAuditStatistics: jest.fn().mockResolvedValue({ total: 0, successRate: 0 }),

  // Chat
  chat: jest.fn().mockResolvedValue({ message: 'Response', streaming: false }),
...
export const MockResponses = {
  ...
  taskDetailResponse: {
    ...
    priority: 1,
    dependencies: [],
    files_to_create: ['src/auth/auth.service.ts', 'src/auth/auth.controller.ts'],
    files_to_modify: ['src/app.module.ts'],
    acceptance_criteria: [
  ...
  budgetReportResponse: {
    ...
    task_spending: [
      { task_id: 'T1', spent_usd: 0.75, budget_usd: 2.0, remaining_usd: 1.25, used_percent: 37.5 },
      { task_id: 'T2', spent_usd: 0.50, budget_usd: 2.0, remaining_usd: 1.50, used_percent: 25.0 },
    ],
  },
    ...
    agents: [
      {
        agent: 'claude',
        available: true,
        last_invocation: '2024-01-01T12:00:00Z',
      ...
      },
      {
        agent: 'cursor',
        available: true,
        last_invocation: '2024-01-01T11:30:00Z',
      ...
      },
      {
        agent: 'gemini',
        available: true,
        last_invocation: '2024-01-01T11:00:00Z',
  ...
  // Chat
  chatResponse: {
    message: 'I understand you want to implement the feature. Let me help you with that.',
    streaming: false,
  },

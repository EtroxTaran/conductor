...
// Mock WebSocket
const createMockWebSocket = (readyState: number = WebSocket.OPEN): jest.Mocked<WebSocket> => ({
  readyState,
  send: jest.fn(),
  close: jest.fn(),
  on: jest.fn(),
...
describe('WebsocketService', () => {
  ...
  describe('broadcastToProject', () => {
    it('should broadcast message to all clients in a project', () => {
      ...
      expect(sentPayload).toHaveProperty('type', WebSocketEventType.STATE_CHANGE);
      expect(sentPayload).toHaveProperty('data', { phase: 2, status: 'in_progress' });
    ...
    it('should include timestamp in message', () => {
      ...
      const sentPayload = JSON.parse(mockClient.send.mock.calls[0][0] as string);

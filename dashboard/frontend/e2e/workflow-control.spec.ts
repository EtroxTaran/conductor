...
test.describe('Workflow Control', () => {
  test('should show start workflow button for not started projects', async ({ page }) => {
  ...
  test('should show pause button for in-progress workflow', async ({ page }) => {
    await setupApiMocks(page, {
      projects: mockProjects.single,
  ...
  test('should handle workflow start action', async ({ page }) => {
    ...
    await overrideApiMock(page, '/api/projects/test-project/workflow/status', mockWorkflowStatus.inProgress);

...
import { setupApiMocks, mockProjects, mockAgents, mockWorkflowHealth } from './fixtures';
...
test.describe('Agent Monitoring', () => {
  ...
  test('should show unavailable agent state', async ({ page }) => {
    await setupApiMocks(page, {
      ...
      agents: [
        { agent: 'claude', available: true, total_invocations: 10, success_rate: 0.9 },
        { agent: 'cursor', available: false, total_invocations: 5, success_rate: 0.0 },
        { agent: 'gemini', available: true, total_invocations: 8, success_rate: 0.85 },
      ],
    });

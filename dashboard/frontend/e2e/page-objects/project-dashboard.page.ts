...
export class ProjectDashboardPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.healthBadge = page.locator('[class*="Badge"][class*="outline"]').last();

    // Action buttons
    this.startWorkflowButton = page.getByRole('button', { name: 'Start Workflow' });
    this.resumeButton = page.getByRole('button', { name: 'Resume' });
    this.pauseButton = page.getByRole('button', { name: 'Pause' });

    // Stats cards
    this.currentPhaseCard = page.locator('[class*="Card"]').filter({ hasText: 'Current Phase' });
    this.taskProgressCard = page.locator('[class*="Card"]').filter({ hasText: 'Task Progress' });
    this.agentsCard = page.locator('[class*="Card"]').filter({ hasText: 'Agents' });
    this.commitsCard = page.locator('[class*="Card"]').filter({ hasText: 'Commits' });
    ...
    // Tab content panels
    this.graphContent = page.locator('[role="tabpanel"][data-state="active"]').filter({ has: page.locator('.react-flow') });
    this.tasksContent = page.locator('[role="tabpanel"]').filter({ hasText: 'Pending' });
    this.agentsContent = page.locator('[role="tabpanel"]').filter({ hasText: 'claude' });
    this.budgetContent = page.locator('[role="tabpanel"]').filter({ hasText: 'spent' });
    this.chatContent = page.locator('[role="tabpanel"]').filter({ has: page.locator('textarea') });
    this.errorsContent = page.locator('[role="tabpanel"]').filter({ hasText: 'Error' });
   ...
   */
  async switchToTab(tabName: 'graph' | 'tasks' | 'agents' | 'budget' | 'chat' | 'errors') {
    const tabMap = {
      graph: this.graphTab,
      tasks: this.tasksTab,
  ...
  async getCurrentPhase(): Promise<string> {
    const content = await this.currentPhaseCard.locator('.text-2xl').textContent();
    return content || '';
  }
  ...
  async getTaskProgress(): Promise<string> {
    const content = await this.taskProgressCard.locator('.text-2xl').textContent();
    return content || '';
  }
  ...
  async isAgentAvailable(agentName: string): Promise<boolean> {
    const agentBadge = this.agentsCard.locator(`[class*="Badge"]:has-text("${agentName}")`);
